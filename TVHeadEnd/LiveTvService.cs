...
namespace TVHeadEnd
{
    public class LiveTvService : ILiveTvService
    {
        ...
        private void ForceDeinterlace(MediaInfo info)
        {
            ...
            _logger.LogInformation("LiveTvService: force video deinterlacing for all channels and recordings is enabled");

            // Applied before the result is cached, so nothing writes through the shallow
            // stream copy into a MediaInfo another media source is still reading.
            foreach (MediaStream i in info.MediaStreams)
            {
        ...
        private void ApplyMediaInfo(MediaSourceInfo mediaSourceInfo, MediaInfo info, long? originalRuntime)
        {
            ...
            // One result feeds more than one media source, so give each its own list.
